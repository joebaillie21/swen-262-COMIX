...
public class ExportAsCSV implements Exporter {
    ...
    public ExportAsCSV(ArrayList<ComicBookComponent> comics) {
        this.comics = comics;
    }

    @Override
    public String toFile() throws IOException {
        String[] header = { "Series", "Issue", "Full Title", "Description", "Publisher", "Release Date", "Format",
                "Added Date", "Authors" };
        ...
        for (int i = 0; i < comics.size(); i++) {
            ...
            String[] entry = { fullSeriesTitle, cb.getIssueNumber(), "", cb.getDescription(),
                    cb.getPublisher().getName(), cb.getPublicationDate(), "Comic", "N/A", authors };
